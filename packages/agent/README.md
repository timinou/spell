...
Stateful agent with tool execution and event streaming. Built on `@spell/pi-ai`.

## Installation

```bash
npm install @spell/pi-agent
```

## Quick Start

```typescript
import { Agent } from "@spell/pi-agent";
...
```typescript
import { Agent, streamProxy } from "@spell/pi-agent";
...
```typescript
import { agentLoop, agentLoopContinue } from "@spell/pi-agent";
