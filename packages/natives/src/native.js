...
const isCompiledBinary = Bun.env.PI_COMPILED ||
...
function selectEmbeddedAddonFile() {
    ...
    if (selectedVariant === "modern") {
        return (embeddedAddon.files.find(file => file.variant === "modern") ??
...
function loadNative() {
    ...
    for (const candidate of runtimeCandidates) {
        try {
            const bindings = logger.time(`native:loadNative:require:${path.basename(candidate)}`, () => require(candidate));
...
function validateNative(bindings, source) {
    ...
    if (missing.length) {
        throw new Error(`Native addon missing exports (${source}). Missing: ${missing.join(", ")}. ` +
