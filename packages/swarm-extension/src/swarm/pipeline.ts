 ...
 */
import type { AgentSource, AuthStorage, ModelRegistry, Settings, SingleResult } from "@spell/pi-coding-agent";
				...
				wave.map(async agentName => {
					...
					try {
						...
						const failResult: SingleResult = {
							...
							outcome: "failed",
							stderr: error,
							resultUri: `agent://swarm-${this.#def.name}-${agentName}-${iteration}`,
							textPreview: error,
							durationMs: 0,
							tokens: 0,
							error,
