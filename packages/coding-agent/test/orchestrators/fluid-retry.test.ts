...
function makeGroups(...tasks: Array<{ id: string; blockers?: string[] }>): TodoGroup[] {
	return [
		{
			id: "phase-1",
			name: "test",
...
describe("computeRetryTaskIds", () => {
	test("returns only failed tasks when no dependents exist", () => {
		const groups = makeGroups({ id: "t-1" }, { id: "t-2" }, { id: "t-3" });
		const result = computeRetryTaskIds(groups, new Set(["t-2"]));
		expect(result).toEqual(new Set(["t-2"]));
	});

	test("includes direct dependents of failed tasks", () => {
		const groups = makeGroups({ id: "t-1" }, { id: "t-2", blockers: ["t-1"] }, { id: "t-3" });
		const result = computeRetryTaskIds(groups, new Set(["t-1"]));
	...
	test("handles multiple failed roots", () => {
		...
		const result = computeRetryTaskIds(groups, new Set(["t-1", "t-2"]));
		expect(result).toEqual(new Set(["t-1", "t-2", "t-3", "t-4"]));
	});
});
