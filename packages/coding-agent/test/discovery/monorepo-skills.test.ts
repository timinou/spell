...
import { clearCache } from "@spell/pi-coding-agent/capability/fs";
import type { Skill } from "@spell/pi-coding-agent/capability/skill";
import type { LoadContext, LoadResult } from "@spell/pi-coding-agent/capability/types";
import { scanSkillsFromDir } from "@spell/pi-coding-agent/discovery/helpers";
...
describe("monorepo skill discovery", () => {
	...
	test("finds skills in ancestor .spell/skills/ directories", async () => {
		// Root has a skill
		writeSkill(path.join(repoRoot, ".spell", "skills"), "root-skill", "From repo root");
		// Sub-project has a skill
		writeSkill(path.join(subProject, ".spell", "skills"), "local-skill", "From sub-project");
		...
		while (true) {
			const result = await scanSkillsFromDir(ctx, {
				dir: path.join(current, ".spell", "skills"),
				providerId: "native",
				level: "project",
			});
			results.push(result);
	...
	test("closest skill wins when same name exists at multiple levels", async () => {
		...
		writeSkill(path.join(repoRoot, ".spell", "skills"), "shared-skill", "Root version");
		writeSkill(path.join(subProject, ".spell", "skills"), "shared-skill", "Local version");
	...
	test("finds skills across multiple ancestor levels", async () => {
		...
		writeSkill(path.join(repoRoot, ".spell", "skills"), "root-skill", "Root");
		writeSkill(path.join(packagesDir, ".spell", "skills"), "packages-skill", "Packages");
		writeSkill(path.join(subProject, ".spell", "skills"), "app-skill", "App");
	...
	test("walk-up stops at repo root and does not find skills above it", async () => {
		...
		writeSkill(path.join(tempDir, ".spell", "skills"), "above-repo-skill", "Above repo");
		// Skill AT the repo root (should be found)
		writeSkill(path.join(repoRoot, ".spell", "skills"), "root-skill", "At repo root");
