...
describe("AutonomyStateTool", () => {
	...
	it("typed schema rejects wrong type", async () => {
		...
		const tool = new AutonomyStateTool(createSession(tempDir));
	...
	it("no schema allows any key-value and registration works through builtin tools", async () => {
		expect(BUILTIN_TOOLS.autonomy_state).toBeDefined();
