...
import type { Skill } from "@spell/pi-coding-agent/sdk";
...
describe("createAgentSession skills option", () => {
	...
	beforeEach(() => {
		...
		// Create skill in .spell/skills/ for native project-level discovery
		skillsDir = path.join(tempDir, ".spell", "skills", "test-skill");
		fs.mkdirSync(skillsDir, { recursive: true });
		originalHome = process.env.HOME;
		tempHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-sdk-home-"));
		process.env.HOME = tempHomeDir;
		const nativeUserSkillsDir = path.join(tempHomeDir, ".spell", "agent", "skills");
...
`,
	...
	it("should still discover project skills when user skills directory is missing", async () => {
		const userAgentDir = path.join(tempHomeDir, ".spell", "agent");
