...
function formatStartupCandidate(
...
function shouldFallbackStartupFailure(message: string): boolean {
  return (
    isStartupAuthFailureMessage(message) ||
    isStartupProviderUnavailableMessage(message)
  );
}

function summarizeStartupFallbackFailures(
  attempts: StartupFallbackAttempt[],
): string {
...
function getReportFindingKey(value: unknown): string | null {
  ...
  const lineStart =
    typeof record.line_start === "number" ? record.line_start : null;
...
export const SUBAGENT_WARNING_NULL_SUBMIT_RESULT =
...
function hasRuntimeVerification(
  opts: RuntimeVerificationOptions | undefined,
): boolean {
...
function formatGateFailures(failures: GateFailure[]): string[] {
  return failures.map(
    (failure) =>
      `- ${failure.gate} not satisfied: expected \`${failure.expected}\`; ${failure.detail}`,
  );
}
...
export function finalizeSubprocessOutput(
  args: FinalizeSubprocessOutputArgs,
): FinalizeSubprocessOutputResult {
  ...
  const hasSubmitResult =
    Array.isArray(submitResultItems) && submitResultItems.length > 0;
  ...
  if (hasSubmitResult) {
    ...
    if (lastSubmitResult?.status === "aborted") {
      ...
      try {
        rawOutput = JSON.stringify(
          { aborted: true, error: lastSubmitResult.error },
          null,
          2,
        );
      } catch {
      ...
      if (submitData === null || submitData === undefined) {
        rawOutput = prependSubagentWarning(
          rawOutput,
    ...
    const fallback = allowFallback
      ? resolveFallbackCompletion(rawOutput, outputSchema)
      : null;
    if (fallback) {
...
function getUsageTokens(usage: unknown): number {
  ...
  const input =
    firstNumberField(record, ["input", "input_tokens", "inputTokens"]) ?? 0;
  const output =
    firstNumberField(record, ["output", "output_tokens", "outputTokens"]) ?? 0;
  const cacheRead =
    firstNumberField(record, ["cacheRead", "cache_read", "cacheReadTokens"]) ??
    0;
  const cacheWrite =
    firstNumberField(record, [
      "cacheWrite",
      "cache_write",
      "cacheWriteTokens",
    ]) ?? 0;
...
function normalizeScopedPath(
  targetPath: string,
  cwd: string,
): { resolved: string; directory: boolean } {
...
function formatScopedPath(entry: {
  resolved: string;
  directory: boolean;
}): string {
...
function intersectScopedPaths(
  ...
  const normalizedBase = basePaths.map((entry) =>
    normalizeScopedPath(entry, cwd),
  );
  const normalizedScope = scopePaths.map((entry) =>
    normalizeScopedPath(entry, cwd),
  );
  for (const scopeEntry of normalizedScope) {
    for (const baseEntry of normalizedBase) {
      if (scopeEntry.resolved === baseEntry.resolved) {
        results.add(
          formatScopedPath(
            scopeEntry.directory && !baseEntry.directory
              ? baseEntry
              : scopeEntry,
          ),
        );
  ...
  return {
    pathsWrite:
      options.basePolicy.pathsWrite.length === 0
        ? []
        : intersectScopedPaths(
            options.basePolicy.pathsWrite,
            scopePaths,
            options.sessionCwd,
          ),
...
export async function runSubprocess(
  ...
  const skipPythonPreflight =
    Array.isArray(toolNames) && !toolNames.includes("python");
    ...
    switch (event.type) {
      ...
      case "tool_execution_start": {
        ...
        progress.currentToolArgs = extractToolArgsPreview(
          (event as { toolArgs?: Record<string, unknown> }).toolArgs ||
            event.args ||
            {},
        );
      ...
      case "tool_execution_end": {
          ...
          if (handler.extractData) {
            ...
            if (
              ...
              if (
                ...
                const executions =
                  (progress.extractedToolData?.bash as
                    | TrackedBashExecution[]
                    | undefined) ?? [];
                const gateResult = await verifyGates({
                  ...
                  baselineHeadCommit:
                    options.runtimeVerification?.baselineHeadCommit,
                });
            ...
            if (extractedData !== undefined && acceptExtractedData) {
              ...
              const findingKey =
                event.toolName === "report_finding"
                  ? getReportFindingKey(extractedData)
                  : null;
              if (findingKey) {
                const existingIndex = existing.findIndex(
                  (item) => getReportFindingKey(item) === findingKey,
                );
                if (existingIndex >= 0) {
        ...
        if (event.toolName === "todo_write") {
          const todoResult = event.result as
            | { details?: { groups?: unknown; phases?: unknown } }
            | undefined;
          const todoGroups =
            todoResult?.details?.groups ?? todoResult?.details?.phases;
        ...
        if (
          maxToolCalls > 0 &&
          !submitResultCalled &&
          progress.toolCount > maxToolCalls
        ) {
      ...
      case "message_update": {
        ...
        const updateContent =
          getMessageContent(event.message) ||
          (event as AgentEvent & { content?: unknown }).content;
      ...
      case "message_end": {
        ...
        const messageUsage =
          getMessageUsage(event.message) ||
          (event as AgentEvent & { usage?: unknown }).usage;
        if (messageUsage && typeof messageUsage === "object") {
          // Only count assistant messages (not tool results, etc.)
          if (role === "assistant") {
            const usageRecord = messageUsage as Record<string, unknown>;
            const costRecord = (
              messageUsage as { cost?: Record<string, unknown> }
            ).cost;
            hasUsage = true;
            accumulatedUsage.input += getNumberField(usageRecord, "input") ?? 0;
            accumulatedUsage.output +=
              getNumberField(usageRecord, "output") ?? 0;
            accumulatedUsage.cacheRead +=
              getNumberField(usageRecord, "cacheRead") ?? 0;
            accumulatedUsage.cacheWrite +=
              getNumberField(usageRecord, "cacheWrite") ?? 0;
            accumulatedUsage.totalTokens +=
              getNumberField(usageRecord, "totalTokens") ?? 0;
            if (costRecord) {
              accumulatedUsage.cost.input +=
                getNumberField(costRecord, "input") ?? 0;
              accumulatedUsage.cost.output +=
                getNumberField(costRecord, "output") ?? 0;
              accumulatedUsage.cost.cacheRead +=
                getNumberField(costRecord, "cacheRead") ?? 0;
              accumulatedUsage.cost.cacheWrite +=
                getNumberField(costRecord, "cacheWrite") ?? 0;
              accumulatedUsage.cost.total +=
                getNumberField(costRecord, "total") ?? 0;
            }
          }
  ...
  const runSubagent = async (): Promise<{
    ...
    const closeActiveSession = async () => {
      ...
      if (activeSession) {
        ...
        activeSession = null;
        try {
          await untilAborted(AbortSignal.timeout(5000), () =>
            session.dispose(),
          );
        } catch {
    ...
    try {
      ...
      checkAbort();
      const modelRegistry =
        options.modelRegistry ?? new ModelRegistry(authStorage);
      await modelRegistry.refresh();
      checkAbort();

      const resolvedCandidates = resolveModelCandidates(
        modelPatterns,
        modelRegistry,
        settings,
      );
      ...
      const mcpProxyTools = options.mcpManager
        ? createMCPProxyTools(options.mcpManager)
        : [];
      const swarmTools =
      ...
      let todoWriteAvailable =
        subagentSettings.get("todo.enabled") &&
        (toolNames === undefined || toolNames.includes("todo_write"));
      const MAX_SUBMIT_RESULT_RETRIES = 3;
      const fallbackAttempts: StartupFallbackAttempt[] = [];

      candidateLoop: for (
        let candidateIndex = 0;
        candidateIndex < startupCandidates.length;
        candidateIndex++
      ) {
          ...
          try {
            const { session } = await createAgentSession({
              ...
              systemPrompt: (defaultBlocks: SystemPromptBlock[]) => {
                ...
                const subagentOverlay = renderPromptTemplate(
                  subagentSystemPromptTemplate,
                  {
                ...
                if (
                  ...
                  const swarmOverlay = renderPromptTemplate(
                    swarmAgentSystemPromptTemplate,
                    {
                      swarmEnabled: true,
                      handoffEnabled: true,
                ...
                const stableBlocks = defaultBlocks.filter(
                  (block) => block.stable !== false,
                );
            ...
            session.sessionManager.appendSessionInit({
              systemPrompt:
                systemPromptText(session.agent.state.systemPrompt) ?? "",
              task,
            ...
            if (
              ...
              startupAssistant?.stopReason === "error"
            ) {
              startupErrorMessage =
                startupAssistant.errorMessage || "Subagent failed";
            }
          } catch (err) {
            startupErrorMessage =
        ...
        if (
          ...
          try {
            const reminder = renderPromptTemplate(submitReminderTemplate, {
              ...
              verificationFailures: formatGateFailures(
                missingVerificationFailures,
              ),
            });
        ...
        if (
          ...
          const verificationMessage = buildMissingVerificationProofMessage(
            missingVerificationFailures,
          );
  ...
  let rawOutput =
    finalOutputChunks.length > 0
      ? finalOutputChunks.join("")
      : outputChunks.join("");
  const submitResultItems = progress.extractedToolData?.submit_result as
    | SubmitResultItem[]
    | undefined;
  const reportFindings = progress.extractedToolData?.report_finding as
    | ReviewFinding[]
    | undefined;
  ...
  const submitResultAbortReason =
    lastSubmitResult?.status === "aborted"
      ? lastSubmitResult.error || "Subagent aborted task"
      : undefined;
  ...
  const missingSubmitResultWarning =
    !hasSubmitResult &&
    rawOutput.startsWith(SUBAGENT_WARNING_MISSING_SUBMIT_RESULT);
  const missingSubmitResultFailed =
    missingSubmitResultWarning && progress.toolCount === 0;
  ...
  const wasAborted =
    abortedViaSubmitResult ||
    (!hasSubmitResult && (done.aborted || signal?.aborted || false));
  ...
  return {
    ...
    todoGroups: progress.todoGroups
      ? cloneTodoGroups(progress.todoGroups)
      : undefined,
