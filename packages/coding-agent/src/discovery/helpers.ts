...
export function parseAgentFields(frontmatter: Record<string, unknown>): ParsedAgentFields | null {
	...
	const scopeRestricted = parseBoolean(frontmatter.scopeRestricted);
	const roster = parseBoolean(frontmatter.roster);
	return { name, description, tools, spawns, model, output, thinkingLevel, blocking, scopeRestricted, roster };
}
...
async function readExtensionModuleManifest(
	...
	const pkg = tryParseJson<{ spell?: ExtensionModuleManifest; pi?: ExtensionModuleManifest }>(content);
