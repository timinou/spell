...
export interface ToolSession {
  ...
  getLastApprovedPlan?: () =>
    | { itemId?: string; title: string; finalPlanFilePath: string }
    | undefined;
  ...
  getBashHistory?: () => ReadonlyArray<
    import("../task/gate-verification").TrackedBashExecution
  >;
  /** Capture a git baseline for the session cwd. Returns null outside a git repo. */
  captureGitBaseline?: () => Promise<
    import("../session/git-baseline").GitBaseline | null
  >;
...
export async function createTools(
  ...
  const requestedTools =
    toolNames && toolNames.length > 0
      ? [...new Set(toolNames.map((name) => name.toLowerCase()))]
      : undefined;
  if (requestedTools && !requestedTools.includes("exit_plan_mode")) {
    requestedTools.push("exit_plan_mode");
  }
  const pythonMode =
    getPythonModeFromEnv() ?? session.settings.get("python.toolMode");
  ...
  const isToolAllowed = (name: string) => {
    ...
    if (name === "render_mermaid")
      return session.settings.get("renderMermaid.enabled");
    if (name === "notebook") return session.settings.get("notebook.enabled");
    if (name === "inspect_image")
      return session.settings.get("inspect_image.enabled");
    ...
    if (name === "search_tool_bm25")
      return session.settings.get("mcp.discoveryMode");
  ...
  const baseEntries =
    filteredRequestedTools !== undefined
      ...
      : [
          ...Object.entries(BUILTIN_TOOLS).filter(([name]) =>
            isToolAllowed(name),
          ),
          ...(includeSubmitResult
            ? ([["submit_result", HIDDEN_TOOLS.submit_result]] as const)
            : []),
  ...
  const baseResults = await Promise.all(
    baseEntries.map(async ([name, factory]) => {
      const tool = await logger.timeAsync(
        `createTools:${name}`,
        factory,
        session,
      );
  ...
  const resolveTool = await logger.timeAsync(
    "createTools:resolve",
    HIDDEN_TOOLS.resolve,
    session,
  );
  if (resolveTool) {
