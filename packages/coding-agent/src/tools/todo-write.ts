...
import {
  resolveArtifactScopeFromArtifactsDir,
  resolveArtifactScopeFromSessionFile,
...
const DelegationSchema = Type.Object({
  ...
  agent: Type.Optional(
    Type.String({ description: "Agent type handling the delegated work" }),
  ),
});
...
const InputTask = Type.Object({
  ...
  notes: Type.Optional(
    Type.String({ description: "Additional context or notes" }),
  ),
  details: Type.Optional(
    Type.String({
      description:
        "Implementation details, file paths, and specifics (shown only when active)",
    }),
  ),
  filesDeps: Type.Optional(
    Type.Array(
      Type.String({
        description:
          "Files this task mutates; used for isolation overlap checks",
      }),
    ),
  ),
  dataContent: Type.Optional(
    Type.String({ description: "Inline content that satisfies a data node" }),
  ),
  artifactPath: Type.Optional(
    Type.String({ description: "Artifact path that satisfies a data node" }),
  ),
  gateCommit: Type.Optional(
    Type.Boolean({ description: "Require commit after completing this task" }),
  ),
  ...
  gateCmd: Type.Optional(
    Type.String({ description: "Command to run for verification" }),
  ),
  gateLlm: Type.Optional(Type.String({ description: "LLM review criteria" })),
  verifyCmd: Type.Optional(
    Type.String({ description: "Recommended verification command" }),
  ),
  ...
  layer: Type.Optional(
    Type.String({ description: "Layer for policy-based gate injection" }),
  ),
});
  ...
  ops: Type.Array(
    Type.Union([
      ...
      Type.Object({
        ...
        group: Type.Optional(
          Type.String({ description: "Group ID, e.g. group-1" }),
        ),
        phase: Type.Optional(
          Type.String({ description: "Legacy group ID alias" }),
        ),
        slug: Type.Optional(
          Type.String({
            description: "Stable slug/ID for the task. Defaults to task-N.",
      ...
      Type.Object({
        ...
        content: Type.Optional(
          Type.String({ description: "Updated task description" }),
        ),
        notes: Type.Optional(
          Type.String({ description: "Additional context or notes" }),
        ),
        ...
        verified: Type.Optional(
          Type.Boolean({
            description:
              "Set true after verifying all gate requirements. Required to complete a gated task.",
          }),
        ),
        deferralFupId: Type.Optional(
          Type.String({
            description:
              "FUP org item ID for deferral tracking. Required when status=abandoned.",
          }),
        ),
      }),
    ]),
...
function resolveTodoContext(
  session: Pick<
    ToolSession,
    "getSessionFile" | "getSessionId" | "getArtifactsDir"
  >,
): ResolvedTodoContext {
  ...
  if (artifactsDir) {
    ...
    return {
      currentSessionId:
        scope.sessionId ?? session.getSessionId?.() ?? "current",
      currentAgentName: scope.agentName,
    };
  }
  const sessionFile = session.getSessionFile();
  if (sessionFile) {
    const scope = resolveArtifactScopeFromSessionFile(
      sessionFile,
...
function buildTodoUri(
  task: Pick<TodoItem, "id" | "kind" | "uri">,
...
function hydrateTodoItem(
  ...
  const hydrated: TodoItem = {
    ...
    children: task.children
      ? task.children.map((child) => hydrateTodoItem(child, context))
      : undefined,
...
function isSatisfiedDataNode(task: TodoItem): boolean {
  ...
  return Boolean(
    task.dataContent ||
    task.artifactPath ||
    task.delegation?.result?.outputPath,
  );
}
...
function cloneTodoDelegation(
  delegation: TodoDelegation | undefined,
): TodoDelegation | undefined {
  if (!delegation) return undefined;
  return {
    ...delegation,
    childGroups: delegation.childGroups
      ? cloneTodoGroups(delegation.childGroups)
      : undefined,
...
function cloneTodoTask(task: TodoItem): TodoItem {
  return {
    ...
    filesDeps: task.filesDeps ? [...task.filesDeps] : undefined,
    children: task.children
      ? task.children.map((child) => cloneTodoTask(child))
      : undefined,
    delegation: cloneTodoDelegation(task.delegation),
  };
}
...
export function injectPolicyGates(
  task: TodoItem,
  policies: TaskPolicy[],
): void {
...
export function queueTodoMutation<T>(
  session: ToolSession,
  action: () => Promise<T>,
): Promise<T> {
  ...
  const next = previous
    .catch(() => undefined)
    .then(() => todoMutationContext.run(true, action));
...
const ORG_DOING_OR_LATER_STATES = new Set([
  "DOING",
  "REVIEW",
  "DONE",
  "BLOCKED",
...
async function transitionOrgItemIfNeeded(
  ...
  const skipStates =
    targetState === "DOING"
      ? ORG_DOING_OR_LATER_STATES
      : ORG_DONE_OR_LATER_STATES;
...
async function applyOrgLifecycleHooks(
  ...
  const previousStatus = new Map(
    flattenTasks(previousGroups).map((task) => [task.id, task.status]),
  );
  ...
  for (const task of flattenTasks(nextGroups)) {
    ...
    if (
      ...
      try {
        const result = await transitionOrgItemIfNeeded(
          projectRoot,
          todoKeywords,
          task.orgItemId,
          "DOING",
        ...
        if (result === "transitioned") {
          notices.push(
            `INFO: Org item ${task.orgItemId} auto-transitioned to DOING.`,
          );
        } else if (result === "not-found") {
          notices.push(
            `WARN: Org item ${task.orgItemId} not found for DOING transition.`,
          );
        }
      } catch (error) {
        const msg = error instanceof Error ? error.message : String(error);
        logger.error(
          "todo_write: failed to auto-transition org item to DOING",
          {
            error,
        ...
        notices.push(
          `WARN: Failed to transition org item ${task.orgItemId} to DOING: ${msg}`,
        );
      }
    }
    if (
      ...
      try {
        ...
        if (result === "transitioned") {
          notices.push(
            `INFO: Org item ${task.orgItemClosingId} auto-transitioned to DONE.`,
          );
        } else if (result === "not-found") {
          notices.push(
            `WARN: Org item ${task.orgItemClosingId} not found for DONE transition.`,
          );
        }
      } catch (error) {
        const msg = error instanceof Error ? error.message : String(error);
        ...
        notices.push(
          `WARN: Failed to transition org item ${task.orgItemClosingId} to DONE: ${msg}`,
        );
      }
    }
  }
...
function resolveTaskRef(
  ...
  return allTasks.find(
    (task) => task.uri === canonical || task.id === resolved.slug,
  );
}
...
export function promoteReadyTasks(
  ...
  for (const task of tasks) {
    if (
      task.status === "in_progress" &&
      hasUnresolvedBlockers(task, tasks, context)
    ) {
      task.status = "pending";
    }
  }
  ...
  for (const task of initialDirectRunning) {
    const canKeep =
      ...
      (isolationMode &&
        task.filesDeps?.length &&
        keptRunning.every((activeTask) => !filesConflict(task, activeTask)));
    if (canKeep) {
  ...
  if (
    !isolationMode &&
    tasks.some(
      (task) => task.status === "failed" || task.status === "gate_failed",
    )
  )
    return;
  ...
  const activeUris = new Set(
    tasks
      .filter((task) => task.status === "in_progress" && !isDelegatedTask(task))
      .map((task) => task.uri!),
  );

  const readyUris = dag
    .getReadyNodeIds(completed)
  ...
  for (const readyUri of readyUris) {
    ...
    if (isolationMode) {
      ...
      const activeTasks = [...activeUris]
        .map((activeUri) =>
          tasks.find((candidate) => candidate.uri === activeUri),
        )
        .filter(
          (activeTask): activeTask is TodoItem => activeTask !== undefined,
        );
      const hasOpaqueConflict =
        activeTasks.length > 0 &&
        (candidateFiles.length === 0 ||
          activeTasks.some(
            (activeTask) => (activeTask.filesDeps?.length ?? 0) === 0,
          ));
      const overlaps = activeTasks.some((activeTask) =>
        dag.hasFileOverlap(readyUri, activeTask.uri!),
      );
...
function collectBlockerGraphWarnings(
  groups: TodoGroup[],
  context: ResolvedTodoContext,
): string[] {
  ...
  const visit = (task: TodoItem): void => {
    ...
    if (visiting.has(task.id)) {
      ...
      if (start >= 0) {
        cycleMessages.add(
          `Circular blockers detected: ${[...stack.slice(start), task.id].join(" -> ")}`,
        );
      }
      return;
    }
...
export function getLatestTodoGroupsFromEntries(
...
function collectCompletedGroupIds(
  previousGroups: TodoGroup[],
  nextGroups: TodoGroup[],
): string[] {
  ...
  for (const group of nextGroups) {
    if (isGroupComplete(group) && !wasComplete.get(group.id))
      completedGroupIds.push(group.id);
  }
  return completedGroupIds;
}

function collectCompletedGatedTasks(
  previousGroups: TodoGroup[],
  nextGroups: TodoGroup[],
): TodoItem[] {
...
export function applyOps(
  ...
  for (const op of ops) {
    switch (op.op) {
      ...
      case "update": {
        ...
        if (op.verificationArtifact !== undefined)
          task.verificationArtifact = op.verificationArtifact;
        ...
        if (op.orgItemClosingId !== undefined)
          task.orgItemClosingId = op.orgItemClosingId;
        if (op.delegation !== undefined)
          task.delegation = cloneTodoDelegation(op.delegation);
        ...
        if (
          op.status === "abandoned" &&
          (!op.deferralFupId || op.deferralFupId.trim() === "")
        ) {
  ...
  for (const task of hydratedTasks) {
    ...
    const pruned = task.blockers.filter(
      (blockerRef) =>
        resolveTaskRef(blockerRef, hydratedTasks, context) !== undefined,
    );
    if (pruned.length !== task.blockers.length)
      task.blockers = pruned.length > 0 ? pruned : undefined;
  }

  const completedGroupIds = collectCompletedGroupIds(
    previousGroups,
    file.groups,
  );
  const completedGatedTasks = collectCompletedGatedTasks(
    previousGroups,
    file.groups,
  );
...
function gateDirectivesForTask(task: TodoItem): string[] {
  ...
  if (task.gateCommit)
    lines.push(
      `REQUIRED: Commit your changes for ${task.id} (${task.content}) before proceeding.`,
    );
  if (task.gateArtifact)
    lines.push(
      `REQUIRED: Verify artifact exists at ${task.gateArtifact} for ${task.id}.`,
    );
  if (task.gateCmd)
    lines.push(`REQUIRED: Run \`${task.gateCmd}\` to verify ${task.id}.`);
  if (task.gateLlm)
    lines.push(
      `ADVISORY: Review ${task.id} against acceptance criteria: ${task.gateLlm}`,
    );
  if (task.verifyCmd)
    lines.push(`RECOMMENDED: Run \`${task.verifyCmd}\` to verify ${task.id}.`);
  return lines;
}
...
export function formatSummary({
  ...
  const remainingTasks = remainingByGroup.flatMap((group) =>
  ...
  if (remainingTasks.length === 0) {
    ...
    const blockedCount = remainingTasks.filter((task) =>
  ...
  if (
    ...
    remainingTasks.every((task) => isTaskBlocked(task, allTasks))
  ) {
    lines.push(
  ...
  const blockedInGroup = current.tasks.filter((task) =>
    isTaskBlocked(task, allTasks),
  ).length;
  const groupBlockedSuffix =
    blockedInGroup > 0 ? `, ${blockedInGroup} blocked` : "";
  lines.push(
  ...
  if (remainingTasks.some((task) => task.status === "gate_failed")) {
    ...
    for (const task of remainingTasks) {
      ...
      if (gateFailures.length === 0) {
        lines.push(
          `gate_failed: ${task.id} "${task.content}" — verification gates were not satisfied`,
        );
        continue;
      }
  ...
  if (completedGroupIds.length > 0) {
    for (const groupId of completedGroupIds) {
      ...
      if (!group) continue;
      const gatedInGroup = group.tasks.filter(
        (task) => hasGate(task) || task.orgItemClosingId,
      );
      if (gatedInGroup.length === 0) continue;
      const actions: string[] = [];
      if (gatedInGroup.some((task) => task.gateCommit))
        actions.push("Commit changes.");
      if (gatedInGroup.some((task) => task.gateArtifact))
        actions.push("Verify artifacts.");
      if (gatedInGroup.some((task) => task.gateCmd || task.verifyCmd))
        actions.push("Run verification commands.");
      lines.push(`\nGroup "${group.name}" complete. ${actions.join(" ")}`);
      const deferredInGroup = group.tasks.filter(
        (task) => task.status === "abandoned" && task.deferralFupId,
      );
      if (deferredInGroup.length > 0) {
        const fupRefs = deferredInGroup
          .map((task) => `${task.id} -> ${task.deferralFupId}`)
          .join(", ");
        lines.push(
          `WARNING: Group "${group.name}" has deferred tasks: ${fupRefs}`,
        );
      }
    }
  }
  ...
  if (pendingVerificationTasks.length > 0) {
    ...
    for (const task of pendingVerificationTasks) {
      lines.push(
        `${task.id} "${task.content}" requires verification before completion:`,
      );
      if (task.gateCmd) lines.push(`  [ ] Run \`${task.gateCmd}\` (gateCmd)`);
      if (task.gateArtifact)
        lines.push(
          `  [ ] Verify artifact at ${task.gateArtifact} (gateArtifact)`,
        );
      if (task.gateCommit) lines.push(`  [ ] Commit changes (gateCommit)`);
      if (task.gateLlm)
        lines.push(`  [i] Advisory review: ${task.gateLlm} (gateLlm)`);
      if (task.orgItemClosingId)
        lines.push(
          `  [i] Verified completion will auto-close org item ${task.orgItemClosingId}.`,
        );
      lines.push("");
      lines.push(
  ...
  if (gateVerificationFailures.length > 0) {
    ...
    for (const failure of gateVerificationFailures) {
      lines.push(
        `${failure.task.id} "${failure.task.content}" could not be verified:`,
      );
      for (const gateFailure of failure.failures) {
        lines.push(
          `  - ${gateFailure.gate}: expected \`${gateFailure.expected}\`, ${gateFailure.detail}`,
        );
      }
      lines.push(
  ...
  if (pendingDeferralTasks.length > 0) {
    ...
    for (const task of pendingDeferralTasks) {
      lines.push(
        `${task.id} "${task.content}" cannot be abandoned without a follow-up item.`,
      );
      lines.push("");
      ...
      lines.push(
        `  org create category=followups title="${suggestedTitle}" body="${bodyLines.join("\n")}"`,
      );
      lines.push("");
...
async function captureDirectWorkBaselines(
  ...
  for (const group of nextGroups) {
    for (const task of group.tasks) {
      ...
      task.gitBaseline = session.captureGitBaseline
        ? await session.captureGitBaseline()
        : null;
    }
  }
}
      ...
      if (task.gateCommit) {
        if (task.gitBaseline === undefined) {
          failures.push({
            ...
            expected: "git commit",
            detail:
              "Git baseline was not captured when the task entered in_progress.",
          });
...
function firstUnresolvedBlockerUri(
  ...
  for (const blockerRef of task.blockers ?? []) {
    const blocker = resolveTaskRef(blockerRef, allTasks, context);
    if (!blocker) continue;
    if (blocker.status !== "completed" && blocker.status !== "abandoned")
      return blocker.uri;
  }
  return undefined;
}
...
function emitTodoGraphEvents(
  ...
  const previousByUri = new Map(
    previousTasks.map((task) => [task.uri!, task] as const),
  );
  for (const task of nextTasks) {
    ...
    const wasBlocked = firstUnresolvedBlockerUri(
      previousTask,
      previousTasks,
      context,
    );
...
export class TodoWriteTool implements AgentTool<
  typeof todoWriteSchema,
  TodoWriteToolDetails
> {
  ...
  async execute(
    ...
    return await queueTodoMutation(this.session, async () => {
      const previousGroups = cloneTodoGroups(
        this.session.getTodoGroups?.() ?? [],
      );
      ...
      const isolationMode = Boolean(
        this.session.settings.get("task.isolation.mode"),
      );
      const {
        file: updated,
        errors,
      ...
      await captureDirectWorkBaselines(
        this.session,
        previousGroups,
        updated.groups,
      );
      ...
      this.session.setTodoGroups?.(
        updated.groups,
        hasReplace ? { reset: true } : undefined,
      );
      const orgLifecycleNotices = await applyOrgLifecycleHooks(
        this.session,
        previousGroups,
        updated.groups,
      );
...
function renderGateBadges(item: TodoItem, uiTheme: Theme): string {
  ...
  if (item.orgItemClosingId)
    badges.push(`[org-closing: ${item.orgItemClosingId}]`);
  if (item.orgItemId && !item.orgItemClosingId)
    badges.push(`[org: ${item.orgItemId}]`);
...
function formatTodoLine(
  ...
  if (allTasks && isTaskBlocked(item, allTasks)) {
    return (
      uiTheme.fg(
        "warning",
        `${prefix}${checkbox.unchecked} ${content} [blocked]`,
      ) + badges
    );
  }
  ...
  switch (item.status) {
    case "completed":
      return (
        uiTheme.fg(
          "success",
          `${prefix}${checkbox.checked} ${chalk.strikethrough(content)}`,
        ) + badges
      );
    case "in_progress": {
      const main =
        uiTheme.fg("accent", `${prefix}${checkbox.unchecked} ${content}`) +
        badges;
    ...
    case "abandoned":
      return (
        uiTheme.fg(
          "error",
          `${prefix}${checkbox.unchecked} ${chalk.strikethrough(content)}`,
        ) + badges
      );
    case "failed":
      return (
        uiTheme.fg("error", `${prefix}${checkbox.unchecked} ${content}`) +
        badges
      );
    case "gate_failed":
      return (
        uiTheme.fg(
          "warning",
          `${prefix}${checkbox.unchecked} ${content} [gate failed]`,
        ) + badges
      );
    default:
      return (
        uiTheme.fg("dim", `${prefix}${checkbox.unchecked} ${content}`) + badges
      );
  }
}
...
export const todoWriteToolRenderer = {
  renderCall(
    args: TodoWriteRenderArgs,
    _options: RenderResultOptions,
    uiTheme: Theme,
  ): Component {
    const count = args.ops?.length ?? 0;
    const label =
      count === 1 ? (args.ops?.[0]?.op ?? "update") : `${count} ops`;
    const text = renderStatusLine(
      { icon: "pending", title: "Todo Write", meta: [label] },
      uiTheme,
    );
  ...
  renderResult(
    ...
    const detailGroups = result.details as
      | { groups?: TodoGroup[]; phases?: TodoGroup[] }
      | undefined;
    const groups = (detailGroups?.groups ?? detailGroups?.phases ?? []).filter(
    ...
    for (const group of groups) {
      if (groups.length > 1) {
        lines.push(
          uiTheme.fg("accent", `  ${uiTheme.tree.hook} ${group.name}`),
        );
      }
