...
import { getToolsDir, getProjectDir, logger, APP_NAME } from "@oh-my-pi/pi-utils";
  ...
  async downloadApk(signal?: AbortSignal): Promise<string> {
    try {
      ...
      const metaRes = await fetch(
        `https://api.github.com/repos/${SPELL_REPO}/releases/latest`,
        {
      ...
      if (!dlRes.ok) {
        throw new Error(
          `APK download error ${dlRes.status}: ${await dlRes.text()}`
        );
      }
