#!/usr/bin/env bun
import { APP_NAME, MIN_BUN_VERSION, VERSION } from "@spell/pi-utils";
/**
 * CLI entry point — registers all commands explicitly and delegates to the
 * lightweight CLI runner from pi-utils.
 */
import { type CommandEntry, run } from "@spell/pi-utils/cli";
...
const commands: CommandEntry[] = [
	...
	{ name: "domain", load: () => import("./commands/domain").then(m => m.default) },
...
async function showHelp(config: import("@spell/pi-utils/cli").CliConfig): Promise<void> {
	const { renderRootHelp } = await import("@spell/pi-utils/cli");
