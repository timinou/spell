...
- `resolveExtensionEntries(dir)` checks, in order:
  - `package.json` manifest (`spell` or `pi`) with `extensions[]` entries,
...
- The `task.isolation.merge` setting controls how isolated changes are integrated back:
  ...
  - **branch**: each task commits to a temp branch (`spell/task/<id>`) via `commitToBranch(...)`, then `mergeTaskBranches(...)` cherry-picks them sequentially onto HEAD. If `git apply` fails inside `commitToBranch`, the error is non-fatal — the agent result is preserved with a `merge failed` status.
...
- Build compiled binary artifact (`dist/spell`):
