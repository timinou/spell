...
export class SettingsList {
    ...
    constructor(items, maxVisible, theme, onChange, onCancel) {
        this.#items = items;
    ...
    #renderMainList(width) {
        ...
        for (let i = startIndex; i < endIndex; i++) {
            ...
            const valueText = this.#theme.value(truncateToWidth(item.currentValue, valueMaxWidth, 2 /* Ellipsis.Omit */), isSelected);
            lines.push(truncateToWidth(prefix + labelText + separator + valueText, width));
        }
        // Add scroll indicator if needed
        if (startIndex > 0 || endIndex < this.#items.length) {
            const scrollText = `  (${this.#selectedIndex + 1}/${this.#items.length})`;
            lines.push(this.#theme.hint(truncateToWidth(scrollText, width - 2, 2 /* Ellipsis.Omit */)));
        }
