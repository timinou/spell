...
import { fuzzyFind } from "@spell/pi-natives";
import { getProjectDir } from "@spell/pi-utils";
const PATH_DELIMITERS = new Set([" ", "\t", '"', "'", "="]);
function buildAutocompleteFuzzyDiscoveryProfile(query, basePath) {
    return {
        query,
        path: basePath,
        maxResults: 100,
...
export class CombinedAutocompleteProvider {
    ...
    #dirCache = new Map();
    #DIR_CACHE_TTL = 2000; // 2 seconds
    constructor(commands = [], basePath = getProjectDir()) {
    ...
    async getSuggestions(lines, cursorLine, cursorCol) {
        ...
        if (atPrefix) {
            ...
            const suggestions = rawPrefix.length > 0
    ...
    async #resolveScopedFuzzyQuery(rawQuery) {
    ...
    async #getCachedDirEntries(searchDir) {
        const now = Date.now();
