...
- Updated environment variable access to use `getEnv()` utility function from `@spell/pi-utils` for consistent handling
...
- Moved `wrapTextWithAnsi` export to `@spell/pi-natives` package
...
- Removed `truncateToWidth`, `sliceWithWidth`, and `extractSegments` functions from public API (now re-exported directly from @spell/pi-natives)
