 ...
 * Also checks for treesit availability (requires Emacs 29+, built with tree-sitter support).
 *
...
export async function detectEmacs(configuredPath?: string): Promise<EmacsDetection> {
	...
	// Check treesit availability only when Emacs meets the minimum version.
	// Uses a sentinel prefix to distinguish "yes"/"no" from other stderr noise.
	let treesitAvailable = false;
	if (meetsMinimum) {
		try {
			const tsResult =
				await Bun.$`${emacsPath} --batch --eval "(message \"TREESIT:%s\" (if (fboundp 'treesit-available-p) (if (treesit-available-p) \"yes\" \"no\") \"no\"))"`
					.quiet()
					.nothrow();
			// emacs --batch writes message output to stderr.
			const tsOutput = tsResult.stderr.toString("utf-8");
			treesitAvailable = tsOutput.includes("TREESIT:yes");
			if (!treesitAvailable) {
				logger.warn("[emacs-detection] treesit not available in this Emacs build", { emacsPath });
			}
		} catch (err) {
			const msg = err instanceof Error ? err.message : String(err);
			logger.warn("[emacs-detection] treesit check failed", { emacsPath, err: msg });
		}
	}
	...
	logger.debug("[emacs-detection] Detection complete", {
		emacsPath,
		version,
		meetsMinimum,
		treesitAvailable,
