#+TITLE: QML Window Screenshot Action
#+STATE: DONE
#+SESSION_ID: 148ef58fe4e5dc11
#+TRANSCRIPT_PATH: [[file:/home/user/.omp/agent/sessions/-code-ora-kika/2026-03-11T20-40-14-995Z_148ef58fe4e5dc11.jsonl]]
#+CUSTOM_ID: DRAFT-001-qml-window-screenshot-action
#+EFFORT: 2h
#+PRIORITY: #B
#+LAYER: backend
...
The QML tool can launch, communicate with, and close Qt windows but cannot capture screenshots of them. This prevents the agent from visually inspecting rendered QML content.

* Approach

Add a =screenshot= action that captures a running window's content and saves it as a PNG. The implementation spans all three layers of the QML stack.

** ITEM Add screenshot command to C++ bridge
:PROPERTIES:
:EFFORT: 45m
:END:

Files: =packages/qml/bridge/windowmanager.h=, =packages/qml/bridge/windowmanager.cpp=

- Add =screenshot= case to =WindowManager::dispatch()=
- New private method =screenshotWindow(const QString &id, const QString &path)=
- Look up the window by id, get =QQuickWindow*= from =engine->rootObjects().first()=
- Call =QQuickWindow::grabWindow()= which returns a =QImage=
- Save via =QImage::save(path, "PNG")=
- Emit event: ={ "type": "screenshot", "id": "...", "path": "/tmp/..." }= on success
- Emit error event on failure (window not found, grab failed, save failed)
- The command JSON: ={ "type": "screenshot", "id": "...", "path": "/tmp/foo.png" }=
- Must =qobject_cast<QQuickWindow*>= since =rootObjects()= returns =QObject*= list
- Grab is async in Qt6: =QQuickWindow::grabWindow()= returns =QImage= synchronously but must be called from the GUI thread (which we are on since dispatch runs on the main thread via QSocketNotifier)

** ITEM Add screenshot types to TypeScript protocol
:PROPERTIES:
:EFFORT: 5m
:END:
...
- Add to =BridgeCommand= union: =| { type: "screenshot"; id: string; path: string }=
- Add to =BridgeEvent= union: =| { type: "screenshot"; id: string; path: string }=

** ITEM Add screenshot method to QmlBridge
:PROPERTIES:
:EFFORT: 10m
:END:
...
- Add =async screenshot(id: string, savePath: string): Promise<string>= method
- Validates window exists, sends screenshot command, waits for screenshot/error event (10s timeout)
- Returns the path on success

Also update =QmlBridge.#handleEvent= to handle the =screenshot= event type (just pass through, the waitFor predicate handles it).

** ITEM Add screenshot action to QML tool
:PROPERTIES:
:EFFORT: 20m
:END:
...
- Add =Type.Literal("screenshot")= to the action union in =qmlSchema=
- =path= parameter is reused (already optional in schema) — for screenshot it's the save destination
- Add =screenshot= case in =execute()= switch:
  - Requires =id=
  - Default save path: =/tmp/omp-qml/screenshot-{id}-{timestamp}.png= if no path given
  - Call =bridge.screenshot(id, savePath)=
  - Return result with the saved path
  - For the result, return the image path so the agent can read it
- Skip remote bridge for now (Android screenshot is a different problem)

** ITEM Update tool prompt
:PROPERTIES:
:EFFORT: 5m
:END:
...
- Add =screenshot= to the actions list: captures window content as PNG, requires =id=, optional =path= for save location

** ITEM Rebuild bridge binary and verify
:PROPERTIES:
:EFFORT: 15m
:END:

- Run =cd packages/qml && bun run build:bridge= to compile the updated C++
- Run =bun check:ts= to verify TypeScript types
- Manual test: write a simple QML, launch, screenshot, verify PNG is saved

* Key Decisions

- Use =QQuickWindow::grabWindow()= (synchronous, GUI-thread) rather than the render-control approach — simpler and sufficient since we're already on the GUI thread
- Save as PNG (lossless, universally supported)
- Default save location under =/tmp/omp-qml/= to keep temp files organized
- Return the file path in the tool result so the agent can =read= the image

* Verification

- =bun check:ts= passes
- Bridge compiles with =bun run build:bridge= in =packages/qml=
- Manual: launch a window, call screenshot, confirm PNG file exists and contains the rendered content
