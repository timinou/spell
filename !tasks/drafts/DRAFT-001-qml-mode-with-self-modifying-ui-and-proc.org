#+TITLE: QML Mode with Self-Modifying UI and Process Restart
#+STATE: ITEM
...
#+CUSTOM_ID: DRAFT-001-qml-mode-with-self-modifying-ui-and-proc
#+EFFORT: 20h
#+PRIORITY: #A
#+LAYER: frontend
...
<file path="/home/user/code/ora/spell/AGENTS.md">
# Development Rules
...
The current working directory is '/home/user/code/ora/spell'.
...
The TUI is a fixed, terminal-bound interface. The user wants Spell to boot into a QML desktop UI where:\n1. The agent can modify the QML files composing its own UI (self-modifying)\n2. QML hot-reloads without restarting the session\n3. Skills ship saved QML interfaces as discoverable panels\n4. A shell QML (MQML) manages loading child QML views safely\n5. Bad QML from the agent must not crash Spell\n6. Agent/tool code hot-reload via process restart with session restore\n\nDesign inspiration: Quickshell (~/.config/quickshell) — modular QML composition.\n\n* Existing Infrastructure (Key Findings)\n\n** Agent runtime is decoupled from TUI\n\n~AgentSession~ exposes: ~subscribe(listener)~ for events, ~prompt(text)~ for input, ~abort()~, ~steer()~. The TUI (~interactive-mode.ts~) is just one consumer. ~print-mode.ts~ (150 lines) shows the minimal driver pattern. ~rpc-mode.ts~ shows full programmatic control.\n\n** QML bridge is production-ready\n\n~packages/qml/~ provides:\n- C++ subprocess (~spell-qml-bridge~) speaking JSON-newline stdio\n- ~QmlBridge~ class: launch/reload/close/sendMessage/screenshot\n- ~QmlWatcher~: fs.watch with 150ms debounce for hot-reload\n- Error events forwarded to TS when QML fails to load\n- Existing skill QML: ~.spell/skills/brand-asset-gallery/gallery.qml~ (402 lines)\n\n** Session persistence exists\n\n~SessionManager~ persists entries as JSONL. ~--resume~ / ~--session~ flags restore sessions. ~setSessionFile()~ loads entries from disk and rebuilds state.\n\n** Mode dispatch in main.ts\n\nLine ~712-757 in main.ts~: branches on ~mode~ — \"rpc\", interactive, print. Adding \"qml\" follows the same pattern.\n\n** QML process lifecycle\n\n~QmlProcess~ spawns ~spell-qml-bridge~ as a child process. It dies when Spell dies. For process restart, we accept full restart (QML shell included) — state is declarative and restored from session + skill discovery.\n\n* Architecture\n\n** Two-layer QML structure\n\n*** Shell QML (MQML)\n\nRoot ApplicationWindow acting as compositor:\n- Sidebar/drawer listing available panels (skills, agent-created, user-saved)\n- Loads child panels into a ~Loader~ component\n- Built-in chat panel (always available) for prompting\n- Catches child panel load errors via ~Loader.status === Loader.Error~\n- Routes ~bridge.messageReceived~ events to active panel\n\n*** Panel QML (views)\n\nIndividual .qml files loaded by the shell via ~Loader { source: panelPath }~.\nOn load failure: shell shows error text + Retry, keeps running.\n\n** Process model\n\n#+begin_example\nspell --qml [initial-message]\n    |\n    +-- createAgentSession()  (same as today)\n    +-- new QmlMode()\n    |     +-- QmlBridge.launch(\"shell\", shell.qml, { props: { panels, session } })\n    |     +-- subscribe(session) -> bridge.sendMessage(events)\n    |     +-- bridge events -> session.prompt() / session.abort() / etc.\n    +-- agent loop\n          +-- tool calls (edit, bash, etc.)\n          +-- qml tool writes .qml -> hot-reload\n#+end_example\n\n** Process restart for code hot-reload\n\n#+begin_example\n[User edits agent/tool TS code]\n    |\n    v\nQML shell detects restart trigger (bridge.send({type:\"restart\"}))\n    or user clicks \"Restart Agent\" button\n    |\n    v\nqml-mode.ts:\n    1. Records current session file path\n    2. Calls session.dispose()\n    3. Re-execs Spell: Bun.spawn([\"spell\", \"--qml\", \"--resume\", sessionFile], { ... })\n    4. Current process exits\n    |\n    v\nNew Spell process boots:\n    1. createAgentSession() with --resume -> restores messages\n    2. QmlMode launches shell.qml (fresh QML process)\n    3. Session continues from where it left off\n#+end_example\n\nThis is a full process restart — both Spell and the QML bridge binary restart. The session JSONL file is the continuity mechanism. The QML shell is stateless (panels discovered from skills, chat history from session events replayed).\n\nAlternative considered: daemon QML process that survives Spell restart. Rejected — adds IPC complexity (socket, reconnection protocol) for marginal UX gain. The restart is fast (<1s) and the QML shell reloads instantly.\n\n* Implementation Plan\n\n** ITEM Create qml-mode.ts\n:PROPERTIES:\n:EFFORT: 4h\n:END:\n\nNew file: ~packages/coding-agent/src/modes/qml-mode.ts~\n\nPattern follows ~print-mode.ts~ (simplest mode):\n- Accept ~AgentSession~\n- Launch QML shell via ~QmlBridge.launch()~\n- Subscribe to ~AgentSessionEvent~, forward as JSON via ~bridge.sendMessage()~\n- Listen for bridge events, dispatch to ~session.prompt/abort/steer~\n- Handle graceful shutdown\n- Handle restart command (re-exec Spell with --resume)\n\nEvent mapping (AgentSessionEvent -> QML):\n- ~message_start/update/end~ -> streaming text chunks\n- ~tool_execution_start/update/end~ -> tool call cards\n- ~agent_start/end~ -> busy indicator\n- ~auto_compaction~ -> compaction notice\n\nQML -> Agent mapping (bridge.send -> session):\n- ~{ type: \"prompt\", text }~ -> ~session.prompt(text)~\n- ~{ type: \"abort\" }~ -> ~session.abort()~\n- ~{ type: \"steer\", text }~ -> ~agent.steer(text)~\n- ~{ type: \"restart\" }~ -> re-exec Spell\n\n** ITEM Write the MQML shell\n:PROPERTIES:\n:EFFORT: 5h\n:END:\n\nNew file: ~packages/coding-agent/src/modes/qml/shell.qml~\n\nStructure:\n- ~SplitView~ with sidebar (panel list) + main area (~Loader~ for active panel)\n- Built-in ~ChatPanel~ as default view\n- Panel registry from ~bridge.props.panels~ (skill QMLs + saved)\n- ~Loader~ with error handling: on ~Loader.Error~, show error + Retry\n- ~bridge.messageReceived~ routes events to active panel\n- \"Restart Agent\" button in sidebar footer\n\nChat panel (built into shell or first panel):\n- ~TextArea~ for input, Enter sends via ~bridge.send({type:\"prompt\"})~\n- ~ScrollView~ with message list, streaming text\n- Tool call cards (collapsible: name + status + result summary)\n- Status bar (model, thinking level, streaming indicator)\n\n** ITEM Wire mode into CLI\n:PROPERTIES:\n:EFFORT: 1h\n:END:\n\nFiles:\n- ~packages/coding-agent/src/cli/args.ts~ — add ~--qml~ flag to ~Args~ interface and ~parseArgs()~\n- ~packages/coding-agent/src/main.ts~ — add ~mode === \"qml\"~ branch at line ~712, calling ~runQmlMode()~\n- ~packages/coding-agent/src/modes/index.ts~ — export ~runQmlMode~\n\n** ITEM Skill QML discovery\n:PROPERTIES:\n:EFFORT: 1h\n:END:\n\nAfter skills load in ~sdk.ts~ or a new utility:\n- For each skill, glob ~*.qml~ in skill directory\n- Build panel manifest: ~{ id, title, path, skillName }~\n- Pass to QmlMode, injected as ~bridge.props.panels~\n- Example: ~brand-asset-gallery/gallery.qml~ appears in sidebar\n\n** ITEM Agent self-modification safety\n:PROPERTIES:\n:EFFORT: 2h\n:END:\n\nWhen agent edits a .qml file currently loaded:\n1. ~QmlWatcher~ triggers reload (already works)\n2. If reload fails, MQML ~Loader~ catches error, shows it in panel\n3. Agent sees error via bridge event, can fix and retry\n4. Strategy: fail-and-show (no pre-validation). Agent iterates fast.\n\nChange needed in ~packages/qml/src/qml-bridge.ts~: when reload fails, include error details in the event so the QML tool result surfaces them to the agent.\n\n** ITEM Process restart mechanism\n:PROPERTIES:\n:EFFORT: 3h\n:END:\n\nIn ~qml-mode.ts~:\n\n#+begin_src typescript\nasync restart(): Promise<never> {\n  const sessionFile = this.session.sessionManager.sessionFile;\n  await this.session.dispose();\n  const args = [\"spell\", \"--qml\", \"--resume\", sessionFile];\n  // Re-exec: replace current process\n  const proc = Bun.spawn(args, {\n    stdio: [\"inherit\", \"inherit\", \"inherit\"],\n    env: process.env,\n  });\n  process.exit(await proc.exited);\n}\n#+end_src\n\nTriggers:\n- QML button: ~bridge.send({type: \"restart\"})~\n- File watcher on key TS source files (optional, later)\n- Could also be triggered by the agent itself\n\nAfter restart, ~--resume~ restores session. New ~QmlMode~ boots fresh QML shell. Chat history replayed from session entries.\n\nEdge cases:\n- Ensure ~session.dispose()~ flushes all pending JSONL entries before exit\n- Handle restart during active agent turn: abort first, then restart\n- If restart fails (e.g. code has syntax error), Spell crashes. User re-runs manually. This is acceptable for dev workflow.\n\n** ITEM Default panels\n:PROPERTIES:\n:EFFORT: 4h\n:END:\n\nShip in ~packages/coding-agent/src/modes/qml/panels/~:\n- ~ChatPanel.qml~ — core chat (prompt + streaming response + tool cards)\n- ~FilePreviewPanel.qml~ — file diffs from edit/write tools\n\nThese are the minimum viable set. Agent can create more panels at runtime.\n\n* Key Files\n\n| File | Action |\n|------|--------|\n| ~packages/coding-agent/src/cli/args.ts~ | Add --qml flag |\n| ~packages/coding-agent/src/main.ts~ | Add mode dispatch (line ~712) |\n| ~packages/coding-agent/src/modes/qml-mode.ts~ | NEW — mode driver + restart |\n| ~packages/coding-agent/src/modes/qml/shell.qml~ | NEW — MQML shell |\n| ~packages/coding-agent/src/modes/qml/panels/ChatPanel.qml~ | NEW — default chat |\n| ~packages/coding-agent/src/modes/qml/panels/FilePreviewPanel.qml~ | NEW — file diffs |\n| ~packages/coding-agent/src/modes/index.ts~ | Export runQmlMode |\n| ~packages/coding-agent/src/sdk.ts~ | Skill QML discovery |\n| ~packages/qml/src/qml-bridge.ts~ | Expose reload errors in events |\n\n* Verification\n\n- ~spell --qml~ launches MQML shell with chat panel\n- Typing a message sends it to agent, streaming response appears\n- Agent edits a .qml file -> panel hot-reloads in shell\n- Agent writes broken QML -> error shown in panel, shell survives\n- Skill QML files appear in sidebar panel list\n- ~spell --qml --resume <session>~ restores previous conversation\n- Clicking \"Restart Agent\" -> Spell restarts, session restored, QML shell relaunches\n- Agent calling ~bridge.send({type:\"restart\"})~ triggers restart cycle\n\n* Dependencies\n\n- ~spell-qml-bridge~ binary must be built: ~cd packages/qml && bun run build:bridge~\n- Qt 6 with QtQuick must be installed\n- No new npm/bun dependencies required"}
