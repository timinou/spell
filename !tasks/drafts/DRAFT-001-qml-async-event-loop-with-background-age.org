#+TITLE: QML Async Event Loop with Background Agent Dispatch
#+STATE: ITEM
#+SESSION_ID: 148eef3c21d2338c
#+TRANSCRIPT_PATH: [[file:/home/user/.omp/agent/sessions/-code-ora-kika/2026-03-11T20-12-36-359Z_148eef3c21d2338c.jsonl]]
#+CUSTOM_ID: DRAFT-001-qml-async-event-loop-with-background-age
#+EFFORT: 4h
#+PRIORITY: #A
#+LAYER: backend
#+RELATED: DRAFT-001-qml-listen-ignores-abortsignal-esc-and-c
...
Currently, when a QML window is launched, the agent must manually call `listen` in a blocking loop. This:
1. Blocks the entire agent loop — agent can't do anything else while waiting for events
2. Requires tedious manual listen → handle → listen cycling
3. Every listen call is a full LLM round-trip just to re-arm the listener
4. The skill prompt has to explicitly instruct the workflow pattern

The user wants: launch a window → events automatically dispatch to background agents → main agent orchestrates freely.

* Design

** Architecture

When `launch` returns, the QmlTool starts a background event loop for that window. Events from the window are delivered as async follow-ups to the main agent (same mechanism as `task` async results), which can then decide to spawn background agents or handle inline.

Key insight: The existing `AsyncJobManager` + `sendCustomMessage(triggerTurn: true)` pattern already solves the "deliver async results back to the agent" problem. We reuse it.

** Event Flow

1. Agent calls `qml launch` → window opens
2. QmlTool starts a persistent background listener (`QmlBridge.waitForEvent` in a loop)
3. When events arrive, QmlTool injects them as a custom message via `session.sendCustomMessage` with `triggerTurn: true`
4. The agent receives the events as a follow-up turn, can process them, spawn background tasks, send messages back to the window, etc.
5. Loop continues until window closes or agent calls `qml close`

This is simpler than spawning a background subagent per event — the main agent already has full context and tool access. The agent itself decides whether to handle inline or delegate to a background task.

** What Changes

*** `listen` action
Remove from schema and implementation. It becomes an internal mechanism, not an agent-facing action. The prompt no longer needs to instruct the listen-loop workflow.

*** `launch` action
After the window is ready, start a background event loop that:
- Calls `waitForEvent` in a loop
- On events, delivers them to the agent via `sendCustomMessage` with `deliverAs: "followUp", triggerTurn: true`
- On window close, stops the loop
- Stores the loop promise + abort mechanism per window so `close`/`dispose` can clean up

*** QmlTool state
Add a `Map<string, AbortController>` for per-window event loops. On `close`, abort the loop for that window. On `dispose`, abort all loops.

*** Prompt (`packages/coding-agent/src/prompts/tools/qml.md`)
Remove `listen` from the action list. Update workflow: launch → events arrive automatically as follow-ups → handle by sending messages or spawning tasks → close when done.

** Key Decisions

- *No per-event subagent spawning.* The main agent receives events and decides what to do. Spawning a subagent per event would be wasteful for simple interactions and lose conversation context. The agent can use `task` tool with `async: true` for heavy work.
- *Reuse sendCustomMessage, not AsyncJobManager.register.* The event loop is not a "job" — it's a persistent listener. We manage it directly in QmlTool and use `sendCustomMessage` for delivery.
- *Need session access.* QmlTool currently only has `ToolSession`. We need a way to send custom messages. Options: (a) add a `sendCustomMessage` callback to ToolSession, (b) use EventBus. EventBus is already on ToolSession. We'll add a new event channel for QML events, and have the sdk.ts handler subscribe to it and call `session.sendCustomMessage`.

** Delivery via EventBus

The QmlTool emits `qml:window:events` on the EventBus with `{ windowId, events }`. In `sdk.ts`, the session subscribes to this channel and calls `session.sendCustomMessage` with `triggerTurn: true` to inject the events into the conversation. This keeps the tool decoupled from session internals.

* Files to Modify

** `packages/coding-agent/src/tools/qml.ts`
- Remove `listen` action from schema and switch
- Add `#eventLoops: Map<string, AbortController>` field
- After launch succeeds, start background event loop that emits on EventBus
- On close, abort the event loop
- On dispose, abort all event loops

** `packages/coding-agent/src/prompts/tools/qml.md`
- Remove `listen` action
- Rewrite workflow: events arrive automatically, agent handles them inline or delegates

** `packages/coding-agent/src/sdk.ts`
- Subscribe to `qml:window:events` on EventBus
- Format events and deliver via `session.sendCustomMessage({ customType: "qml-events", ... }, { deliverAs: "followUp", triggerTurn: true })`

** `packages/coding-agent/src/utils/event-bus.ts` (if needed)
- Add channel constant for QML events (or just use string literal)

** `packages/coding-agent/src/tools/index.ts`
- Remove `listen` from QmlTool schema type (already handled by qml.ts change)

* Edge Cases

- *Multiple windows:* Each window gets its own event loop. Events include windowId so the agent knows which window they're from.
- *Rapid events:* The event loop drains all queued events per wake-up (waitForEvent returns all buffered events). Events are batched into a single follow-up message.
- *Window closed externally:* The event loop detects the close event and terminates. The follow-up message includes a `[closed]` indicator.
- *Agent not ready for follow-up:* `sendCustomMessage` with `triggerTurn` queues the message — it will be delivered when the current turn completes.
- *Remote bridge (Android):* Same pattern applies — RemoteQmlBridge also has `waitForEvent`. The event loop works identically.

* Verification

- Launch a QML window, verify events arrive as follow-up messages without manual `listen` calls
- Verify window close terminates the event loop cleanly
- Verify dispose cleans up all event loops
- Verify remote bridge path works the same
- Run `bun check:ts` to confirm type safety
- Run `bun lint:ts` to confirm lint passes

* Sub-tasks

** ITEM Remove listen action from QmlTool schema and implementation
:PROPERTIES:
:EFFORT: 30m
:END:

Remove the `listen` literal from the schema union, remove the `case "listen"` block, remove listen-related types from QmlToolDetails.

** ITEM Add background event loop to launch action
:PROPERTIES:
:EFFORT: 1h
:END:

Add `#eventLoops` map, start loop after launch, emit events on EventBus. Handle both local and remote bridge paths. Clean up on close/dispose.

** ITEM Wire EventBus to session follow-ups in sdk.ts
:PROPERTIES:
:EFFORT: 45m
:END:

Subscribe to QML event channel, format events, deliver via sendCustomMessage with triggerTurn.

** ITEM Update QML tool prompt
:PROPERTIES:
:EFFORT: 30m
:END:

Rewrite qml.md to remove listen, describe new automatic event delivery, update workflow section.

** ITEM Verify and test
:PROPERTIES:
:EFFORT: 30m
:END:

Run type checks, lint. Manual verification with a test QML window if possible.
