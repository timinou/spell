#+TITLE: Add org protocol for plan references
#+STATE: DONE
#+SESSION_ID: 14905a1e0a31d7ed
#+TRANSCRIPT_PATH: [[file:/home/user/.spell/agent/sessions/-code-ora-spell/2026-03-12T22-38-03-817Z_14905a1e0a31d7ed.jsonl]]
#+CUSTOM_ID: DRAFT-001-add-org-protocol-for-plan-references
#+EFFORT: 3h
#+PRIORITY: #B
#+LAYER: backend
...
When a plan is created via =org create= and approved via =exit_plan_mode=, the execution session references it as =local://TITLE.md= — a filesystem copy of the org item body. The org item is the source of truth, but the execution session doesn't know about it. This creates:
- A redundant copy of plan content in the local:// filesystem
- Loss of org item identity (CUSTOM_ID) in the execution session
- Inconsistency: the plan lives in org but is referenced via a different protocol

* Approach

Add an =org://= internal URL protocol handler so plans (and org items generally) can be referenced directly by CUSTOM_ID.

** URL shape

=org://ITEM-ID= (e.g. =org://PLAN-003-auth-refactor=)

CUSTOM_IDs are globally unique across categories, so no category prefix needed. Bare =org://= is not supported initially (could list items later).

** Lifecycle change

Current flow:
1. Agent creates draft org item → =DRAFT-003-auth-refactor=
2. =exit_plan_mode(title, itemId)= → returns =finalPlanFilePath: "local://TITLE.md"=
3. On approval: =finalizePlanDraft= marks draft DONE, creates active item =PLAN-003-auth-refactor=
4. Plan body copied to =local://TITLE.md=, execution session references that

New flow:
1. Same
2. =exit_plan_mode(title, itemId)= → returns =finalPlanFilePath= unchanged (still local:// for the tool result shape)
3. On approval: =finalizePlanDraft= returns active item ID =PLAN-003-auth-refactor=
4. No local:// copy needed; =setPlanReferencePath("org://PLAN-003-auth-refactor")=; prompt template receives =org://PLAN-003-auth-refactor=

Key: the active item ID is only known after =finalizePlanDraft= runs, so the =org://= reference is set in =#approvePlan=, not in =exit_plan_mode=.

For file-backed plans (org disabled), the existing local:// flow is unchanged.

* Files to create

** ITEM =src/internal-urls/org-protocol.ts= — OrgProtocolHandler
:PROPERTIES:
:EFFORT: 45m
:END:

Implements =ProtocolHandler= for scheme ="org"=.
...
=resolve(url)=:
- Extract CUSTOM_ID from =url.rawHost= (the host segment is the item ID)
- Call =findItemById= from =@oh-my-pi/pi-org= (reusing =buildOrgConfig= from =plan-mode/org-plan.ts=)
- Return item body as =text/markdown= content
- If item not found, throw descriptive error
- If org disabled, throw "org:// requires org.enabled"

Export from =src/internal-urls/index.ts=.

** ITEM Register handler in =src/sdk.ts=
:PROPERTIES:
:EFFORT: 10m
:END:

Register =OrgProtocolHandler= alongside other handlers. Needs =settings= and =getCwd= closures already available in scope.

** ITEM Update =#approvePlan= in =src/modes/interactive-mode.ts=
:PROPERTIES:
:EFFORT: 30m
:END:

When =finalizePlanDraft= succeeds and returns an =activeOrgItemId=:
- Skip the =Bun.write(newLocalPath, planContent)= step
- Set =planReferencePath= to =org://ACTIVE_ITEM_ID= instead of =options.finalPlanFilePath=
- Pass =org://ACTIVE_ITEM_ID= as =finalPlanFilePath= to the prompt template

Fall back to existing local:// flow when org is disabled or finalization fails.

** ITEM Update =#buildPlanReferenceMessage= in =src/session/agent-session.ts=
:PROPERTIES:
:EFFORT: 30m
:END:

Currently hardcodes =resolveLocalUrlToPath=. Change to:
- If =planReferencePath= starts with ="org://"=, resolve via the org infrastructure (import =resolvePlanDraftItem= or =findItemById= + =buildOrgConfig=)
- Otherwise, use existing =resolveLocalUrlToPath= path

The session doesn't have the internal router, so call org reading functions directly (same pattern as =resolvePlanDraftItem=). The session does have =settings=.

** ITEM Update =approved-plan.ts= — skip rename for org-backed plans
:PROPERTIES:
:EFFORT: 15m
:END:

=renameApprovedPlanFile= currently asserts =local://= prefix. For org-backed plans, the rename is unnecessary — the caller already skips it when =options.orgItem= is set (line 795). Verify this path is correct and no rename is attempted for org-backed plans.
...
=plan-mode-approved.md= references =finalPlanFilePath= — this will now be =org://ITEM-ID= for org-backed plans. The template already uses it as a plain string, so no change needed. Verify =plan-mode-reference.md= similarly works.

** ITEM Update bash URL resolution in =bash-skill-urls.ts=
:PROPERTIES:
:EFFORT: 15m
:END:

Check if =org://= URLs need resolution in bash commands (like =local://= and =skill://= are). If bash commands could reference =org://ITEM-ID=, add resolution support. Otherwise, explicitly reject with a clear error.
...
- Unit test for =OrgProtocolHandler=: resolves item by ID, handles missing item, handles org disabled
- Update =exit-plan-mode.test.ts= if return shape changes
- Test that =#approvePlan= sets =org://= reference path when org finalization succeeds

* Verification

- =bun check:ts= passes
- =bun lint:ts= passes
- New org-protocol tests pass
- Manual verification: in plan mode with org enabled, approve a plan → execution session prompt shows =org://ITEM-ID= reference, not =local://=
- File-backed fallback (org disabled) still works unchanged
