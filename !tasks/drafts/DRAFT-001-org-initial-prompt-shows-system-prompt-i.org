#+TITLE: Org Initial Prompt Shows System Prompt Instead Of User Message
#+STATE: DONE
#+SESSION_ID: 14905dc5dcd9cf67
#+TRANSCRIPT_PATH: [[file:/home/user/.spell/agent/sessions/-code-ora-spell/2026-03-12T22-54-02-099Z_14905dc5dcd9cf67.jsonl]]
#+CUSTOM_ID: DRAFT-001-org-initial-prompt-shows-system-prompt-i
#+EFFORT: 30m
#+PRIORITY: #B
#+LAYER: backend
...
When creating org items (especially plan drafts), the =* Initial Prompt= section contains the full agent system prompt instead of the first human message from the session. The system prompt is huge and not useful in org files. The user wants the initial human message that triggered the plan.

* Root Cause

=OrgSessionContext.systemPrompt= is documented as "Full agent system prompt" and =buildSessionContext()= in =packages/coding-agent/src/tools/org.ts= passes =session.getSystemPrompt()=. The org-writer then writes this under =* Initial Prompt=.

* Approach

Rename the field and plumb the first user message instead.

** ITEM Rename OrgSessionContext.systemPrompt to initialMessage
:PROPERTIES:
:EFFORT: 10m
:END:

Files:
- =packages/org/src/types.ts= — rename field =systemPrompt= to =initialMessage=, update JSDoc
- =packages/org/src/org-writer.ts= — change =session.systemPrompt= refs to =session.initialMessage=, rename heading from =* Initial Prompt= to =* Initial Message=
- =packages/org/test/org-writer.test.ts= — update test descriptions and assertions

** ITEM Wire first user message through ToolSession
:PROPERTIES:
:EFFORT: 10m
:END:

Files:
- =packages/coding-agent/src/tools/index.ts= — replace =getSystemPrompt= with =getFirstUserMessage= on =ToolSession= (or add it alongside, since =getSystemPrompt= is used elsewhere for other purposes)
- =packages/coding-agent/src/tools/org.ts= — change =buildSessionContext()= to pass =session.getFirstUserMessage?.()= as =initialMessage=

Actually, =getSystemPrompt= is used by extensions and other things — keep it. Just add =getFirstUserMessage=.

- =packages/coding-agent/src/tools/index.ts= — add =getFirstUserMessage?: () => string= to =ToolSession=
- =packages/coding-agent/src/tools/org.ts= — change =buildSessionContext= to use =getFirstUserMessage= for =initialMessage= field

** ITEM Wire getFirstUserMessage in all ToolSession construction sites
:PROPERTIES:
:EFFORT: 10m
:END:

The =ToolSession= object is constructed in multiple places. The first user message logic already exists as =#getFirstUserMessageText()= in =interactive-mode.ts=. Need to wire it through.

Key construction sites (grep =getSystemPrompt:= to find them, add =getFirstUserMessage= alongside):
- =packages/coding-agent/src/session/agent-session.ts= (line ~2208)
- =packages/coding-agent/src/modes/interactive-mode.ts= (via agent-session)
- =packages/coding-agent/src/sdk.ts= (line ~853)
- =packages/coding-agent/src/task/executor.ts= (line ~1060)
- =packages/coding-agent/src/modes/print-mode.ts= (line ~84)
- =packages/coding-agent/src/modes/rpc/rpc-mode.ts= (line ~361)

The =AgentSession= is the primary one — it constructs the =ToolSession= passed to tools. The session has access to =this.messages= so it can implement the first-user-message lookup directly.

* Verification

- =bun check:ts= passes
- =bun test packages/org/test/org-writer.test.ts= passes with updated assertions
- Create an org item in a =writeInitialPrompt: true= category and verify the =* Initial Message= section contains the user message, not the system prompt
