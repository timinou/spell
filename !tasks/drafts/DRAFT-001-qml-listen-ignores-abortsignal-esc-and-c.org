#+TITLE: QML listen ignores AbortSignal - esc and close hang
#+STATE: ITEM
...
#+CUSTOM_ID: DRAFT-001-qml-listen-ignores-abortsignal-esc-and-c
#+EFFORT: 1h
#+PRIORITY: #B
#+LAYER: backend
...
When the agent is in a `listen` call on a QML window, pressing Escape (which should abort the tool) does nothing. The process hangs until the 10-minute timeout, even if the QML window is closed externally.

* Root Cause

Two layers both ignore the abort signal:

1. `packages/coding-agent/src/tools/qml.ts` — the `execute` method receives `_signal?: AbortSignal` but never passes it to `waitForEvent`. The underscore prefix indicates it was intentionally unused.

2. `packages/qml/src/qml-bridge.ts` — `waitForEvent(id, timeoutMs)` has no AbortSignal parameter at all. It creates a Promise.withResolvers and only resolves on bridge events or a 10-minute setTimeout.

So pressing Escape fires the AbortSignal, but nothing listens for it. The Promise just sits there.

Additionally: if the QML window is closed externally (user clicks X), the `closed` event should trigger the listener — and it does IF the bridge process is healthy. But if the bridge subprocess itself dies or the stdin/stdout pipe breaks, the listener is also orphaned.

* Fix

** `QmlBridge.waitForEvent` — accept optional AbortSignal
Add `signal?: AbortSignal` parameter. Wire it to call `done([])` (or reject) when aborted. Clean up the timeout and process listener on abort.

** `QmlTool.execute` listen case — pass signal through
Pass the `_signal` to `bridge.waitForEvent(id, timeout, signal)`.

* Relationship to DRAFT-001-qml-async-event-loop

The async event loop plan (DRAFT-001) removes `listen` as an agent-facing action, which eliminates this bug for users. However, the internal background event loop introduced by that plan also calls `waitForEvent` in a loop and needs AbortSignal support for clean shutdown on `close`/`dispose`. So this fix is still needed — it becomes infrastructure for the async plan rather than a user-facing fix.

Recommend: implement this AbortSignal fix as part of DRAFT-001 (sub-task of the background event loop work), not as a standalone change.

* Files

- `packages/qml/src/qml-bridge.ts` — add signal param to `waitForEvent`
- `packages/coding-agent/src/tools/qml.ts` — pass signal through (or remove listen action per DRAFT-001)
