#+TITLE: Silent QML events
#+STATE: DONE
#+SESSION_ID: 1490041544bbb075
#+TRANSCRIPT_PATH: [[file:/home/user/.omp/agent/sessions/-code-ora-kika/2026-03-12T16-22-10-450Z_1490041544bbb075.jsonl]]
#+CUSTOM_ID: DRAFT-001-silent-qml-events
#+EFFORT: 2h
#+PRIORITY: #B
#+LAYER: backend
...
QML window events all flow through one path: ~QmlTool.#startEventLoop~ emits to ~QML_EVENTS_CHANNEL~, and ~sdk.ts~ delivers every batch as a ~followUp~ with ~triggerTurn: true~ and ~display: true~. This means every ~url_changed~, every fontconfig stderr warning, every "Lit is in dev mode" message triggers a full agent turn, pollutes the chat, and burns tokens.

Evidence from session ~148ff6d086b3ce4b~: 18+ noisy events (9 repeated ~url_changed~ to the same URL, fontconfig errors, CSP violations, Lit dev mode warnings) each triggering a separate agent turn.

* Design

Two-level classification: events are either "silent" or "loud". Silent events are recorded in the session for debugging but don't trigger agent turns or display in chat.

** Classification rules

1. *QML-side opt-in*: ~bridge.send({ ..., silent: true })~ — the QML app explicitly marks low-value events
2. *Auto-classification in ~#startEventLoop~*: pattern-match known noisy events:
   - ~url_changed~ events → silent (navigation noise)
   - ~stderr~ events matching known harmless patterns (fontconfig, CSP frame-ancestors, "dev mode") → silent
3. *Everything else* → loud (user clicks, element selections, JS errors, close events, unknown event types)

** Delivery mechanism

- *Silent events*: call ~sendCustomMessage~ with ~display: false~ and ~triggerTurn: false~ (no ~deliverAs: "followUp"~). They land in the session transcript for replay/debugging but don't trigger turns or show in chat.
- *Loud events*: current behavior unchanged — ~display: true~, ~deliverAs: "followUp"~, ~triggerTurn: true~.
- *Batching*: when a loud event arrives, include a count of accumulated silent events since the last loud event as a one-line summary prefix (e.g., "6 silent events suppressed (3x url_changed, 2x stderr, 1x heartbeat)"). This keeps the agent aware that activity happened without dumping payloads.

** Deduplication

Adjacent identical events (same ~name~ + same ~payload~) should be collapsed into a count. E.g., 5 consecutive ~url_changed~ to ~/login~ becomes one silent event with ~count: 5~.

* Files to modify

** ~packages/coding-agent/src/tools/qml.ts~
- Add ~#classifyEvent(event)~ method returning ~"silent" | "loud"~
- Modify ~#startEventLoop~ to split events into silent/loud batches
- Track silent event accumulator between loop iterations
- Emit silent events with ~display: false~, no turn trigger
- On loud events, prepend silent event summary

** ~packages/coding-agent/src/sdk.ts~
- Update the ~QML_EVENTS_CHANNEL~ handler to respect a new ~silent~ flag on the payload, using ~display: false~ and omitting ~triggerTurn: true~ / ~deliverAs: "followUp"~ for silent events

** ~packages/coding-agent/src/tools/qml.ts~ (QmlWindowEventsPayload)
- Add ~silent?: boolean~ to ~QmlWindowEventsPayload~

* Edge cases

- JS errors in stderr (~TypeError~, ~SyntaxError~, ~ReferenceError~) must remain loud — they indicate real bugs
- If ALL events in a batch are silent, emit a single silent custom message (no turn)
- If a batch has a mix, the loud ones trigger a turn and silents are summarized
- ~close~ events are always loud
- Unknown/new event types default to loud (fail-open for visibility)

* Verification

- Launch a QML window with a WebEngineView that navigates (produces url_changed)
- Verify url_changed events appear in session JSONL with ~display: false~
- Verify JS errors still trigger agent turns
- Verify silent event summary appears when a loud event follows silents
- Run ~bun check:ts~ to confirm types
