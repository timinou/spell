...
- Escape hatch: set `PI_KNOWLEDGE_WORKER=inprocess` to bypass the daemon socket and fall back to the in-process `WarmEngine`. Useful for CI, offline development, and tests. In this mode, RPC errors are silent (the old behaviour); in the default `Daemon` mode, RPC errors are propagated as query failures.
