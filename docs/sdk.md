...
The SDK is the in-process integration surface for `@spell/pi-coding-agent`.
...
```bash
bun add @spell/pi-coding-agent
```

## Entry points

`@spell/pi-coding-agent` exports the SDK APIs from the package root (and also via `@spell/pi-coding-agent/sdk`).
...
```ts
import { createAgentSession } from "@spell/pi-coding-agent";
...
```ts
import { createAgentSession, SessionManager } from "@spell/pi-coding-agent";
...
```ts
import { SessionManager } from "@spell/pi-coding-agent";
