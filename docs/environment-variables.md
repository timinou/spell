...
Most runtime lookups use `$env` from `@spell/pi-utils` (`packages/utils/src/env.ts`).
...
These are consumed via `@spell/pi-utils/dirs` and affect where coding-agent stores data.
