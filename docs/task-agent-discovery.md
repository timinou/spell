...
`discovery.ts` header comments still mention `.pi` and do not mention `.codex`/`.gemini`. Actual runtime order is driven by `src/config.ts` and currently uses `.spell`, `.claude`, `.codex`, `.gemini`.
...
- Higher-priority source family overrides lower (`.spell` before `.claude`, etc.).
