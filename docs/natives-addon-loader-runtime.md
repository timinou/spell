...
This document deep-dives the addon loading/validation layer in `@spell/pi-natives`: how `native.ts` decides which `.node` file to load, when embedded payload extraction runs, and how startup failures are reported.
...
- **Core directories**:
  ...
  - `userDataDir` fallback:
    - Windows: `%LOCALAPPDATA%/spell` (or `%USERPROFILE%/AppData/Local/spell`).
...
- reinstall hint (`bun install @spell/pi-natives`),
