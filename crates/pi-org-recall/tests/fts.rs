...
/// 4. Re-indexing replaces existing document (old content no longer searchable).
#[test]
...
let _expected = cache_dir.path().join("somehash/fts");
	...
	idx.index(&[mk_item("EP-1", "Test", "episode", Some("body"))]).unwrap();
	drop(idx);
	...
	assert!(hash_entry.path().join("fts").exists(),
		"expected fts subdirectory under hash dir");
}
