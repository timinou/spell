...
use crate::{ast::NamePayload, dialect::NameLexer};
...
impl NameLexer for TsNameLexer {
	fn parse<'s>(&self, input: &mut &'s str) -> winnow::Result<NamePayload> {
		...
		if segments.iter().any(|s| matches!(s, TsSegment::Quoted(_))) {
			Ok(NamePayload::Quoted(render_ts_name(&TsName { segments })))
		} else {
	...
	fn matches(&self, n: &NamePayload, node: tree_sitter::Node<'_>, src: &str) -> bool {
		match n {
			NamePayload::Raw(target) => self.matches_raw(target, node, src),
...
impl TsNameLexer {
	fn matches_raw(&self, target: &str, node: tree_sitter::Node<'_>, src: &str) -> bool {
		...
		if node.kind() == "export_statement" {
			...
			for child in node.children(&mut cursor) {
				if child.kind() != "export_statement"
					&& self.matches(&NamePayload::Raw(target.to_string()), child, src)
				{
					return true;
				}
			}
...
fn parse_segment(input: &mut &str) -> winnow::Result<TsSegment> {
	if input.starts_with('`') {
		...
		*input = &input[consumed..];
		return Ok(TsSegment::Quoted(buf));
	}
	if input.starts_with('#') {
	...
	while let Some(n) = stack.pop() {
		if n.kind() == "if_statement" {
			...
			for child in n.children(&mut cursor) {
				...
				// Heuristic: the first parenthesized_expression or field named
				// condition
			}
			...
			AnchorPattern {
				...
				matcher: |n, _s| {
					n.kind() == "expression_statement"
						&& n.parent().map_or(false, |p| p.kind() == "program")
				},
			},
		],
		qualifiers:   vec![
			...
			QualifierSpec {
				name:       "decorators",
				applies_to: vec!["class_declaration".into(), "method_definition".into()],
