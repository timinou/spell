...
async fn multi_commit_atomically_records_all_files() {
	...
	b.send(&ClientMessage::Subscribe {
		files: vec![PathBuf::from("/tmp/x.ts"), PathBuf::from("/tmp/y.ts")],
	})
	.await;
...
async fn multi_commit_rejects_unknown_txn() {
	...
	client
		.send(&ClientMessage::MultiCommit { txn_id: "nonexistent".into(), files: vec![] })
		.await;
...
async fn multi_commit_emits_single_event_with_all_files() {
	...
	b.hello(std::process::id()).await;
	b.send(&ClientMessage::Subscribe {
		files: vec![PathBuf::from("/tmp/p.ts"), PathBuf::from("/tmp/q.ts")],
	})
	.await;
	// Drain peer_joined
	let _ = a.recv_within(Duration::from_millis(200)).await;
	...
	let multi: Vec<_> = events
		.into_iter()
		.filter_map(|e| {
			if matches!(&e, ServerMessage::MultiPeerCommitted { .. }) {
				Some(e)
			} else {
				None
			}
