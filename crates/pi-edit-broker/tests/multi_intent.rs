...
async fn multi_intent_grants_when_all_free() {
	...
	client
		.send(&ClientMessage::MultiIntent { txn_id: "txn-1".into(), files, ttl_ms: 10_000 })
		.await;
...
async fn multi_intent_releases_partial_grants_on_conflict() {
	...
	b.send(&ClientMessage::MultiIntent { txn_id: "txn-2".into(), files, ttl_ms: 10_000 })
		.await;
	...
	assert!(
		matches!(ack2, ServerMessage::IntentAck { granted: true, .. }),
		"A should still hold ::bar"
	);
	broker.shutdown().await;
}
...
async fn multi_intent_ttl_expires_unclaimed_grants() {
	...
	assert!(
		matches!(ack2, ServerMessage::IntentAck { granted: true, .. }),
		"intent should be grantable after ttl expiry"
	);
	broker.shutdown().await;
}
...
async fn multi_intent_supersedes_existing_single_intent() {
	...
	client
		.send(&ClientMessage::MultiIntent { txn_id: "txn-4".into(), files, ttl_ms: 10_000 })
		.await;
	let ack = client.recv().await.expect("MultiIntentAck");
	match ack {
