...
// FUP: TS-side e2e harness via packages/coding-agent/test/codepath-e2e/ deferred to follow-up FUP.

use std::path::PathBuf;

use pi_natives::code_path::napi::{execute_code_path_inner, CodePathChunk, CodePathTaskOptions, NodeRefDto, ContentDto};
...
mod symbol_query_tests {
    ...
    fn tsx_symbol_query_returns_component_node() {
        ...
        let ok = nodes.iter().any(|n| {
            matches!(n.kind.as_str(), "§variable_declarator" | "§arrow_function" | "§function_declaration")
        });
    ...
    fn markdown_symbol_query_returns_heading_node() {
        ...
        let ok = nodes.iter().any(|n| {
            matches!(n.kind.as_str(), "§section" | "§atx_heading")
        });
...
mod qualifier_tests {
    ...
    fn resolve_qualified<N: pi_code_path::dialect::NameLexer>(path: &std::path::Path, target: &str, lexer: &N) -> Vec<pi_code_path::types::NodeRef> {
    ...
    fn typescript_qualifier_body_returns_method_body() {
        ...
        let results = resolve_qualified(&path, "sample.ts::Greeter.greet#body", &pi_code_path::dialects::typescript::TsNameLexer);
    ...
    fn rust_qualifier_name_returns_identifier() {
        ...
        let results = resolve_qualified(&path, "sample.rs::Point#name", &pi_code_path::dialects::rust::RustNameLexer);
        assert_eq!(results.len(), 1);
        let text = content_text(&results[0]);
    ...
    fn python_qualifier_body_returns_async_method_body() {
        ...
        let results = resolve_qualified(&path, "sample.py::Service.fetch#body", &pi_code_path::dialects::python::PyNameLexer);
        assert_eq!(results.len(), 1);
        let text = content_text(&results[0]);
    ...
    fn go_qualifier_body_returns_receiver_method_body() {
        ...
        let results = resolve_qualified(&path, "sample.go::Rectangle.Area#body", &pi_code_path::dialects::go::GoNameLexer);
        assert_eq!(results.len(), 1);
        let text = content_text(&results[0]);
    ...
    fn haskell_qualifier_body_returns_function_body() {
        ...
        let results = resolve_qualified(&path, "sample.hs::factorial#body", &pi_code_path::dialects::haskell::HsNameLexer);
        assert_eq!(results.len(), 1);
        let text = content_text(&results[0]);
    ...
    fn html_qualifier_inner_html_excludes_tags() {
        ...
        let results = resolve_qualified(&path, "sample.html::header#innerHTML", &pi_code_path::dialects::html::HtmlNameLexer);
    ...
    fn css_qualifier_selector_returns_selector_text() {
        ...
        let results = resolve_qualified(&path, "sample.css::.btn#selector", &pi_code_path::dialects::css::CssNameLexer);
    ...
    fn markdown_qualifier_first_para_returns_paragraph() {
        ...
        let results = resolve_qualified(&path, "sample.md::Introduction#first-para", &pi_code_path::dialects::mdorg::MdNameLexer);
        assert!(!results.is_empty());
        let text = content_text(&results[0]);
...
mod routing_feature_tests {
    ...
    fn cancellation_aborts_mid_walk() {
        ...
        cancel.emplace_abort_token().abort(pi_natives::task::AbortReason::User);
    ...
    fn threshold_respects_large_file() {
        ...
        let node = nodes.iter().find(|n| n.content.is_some()).expect("expected content node");
        let content = node.content.as_ref().unwrap();
        if content.kind == "text" {
            // If text, it must be under threshold. With 300 KiB file, this branch is unlikely.
    ...
    fn auto_raw_on_bare_path_returns_content() {
        ...
        let nodes = nodes_from(&chunks);
        let node = nodes.iter().find(|n| n.content.is_some()).expect("expected content");
        let text = node.content.as_ref().unwrap().text.clone().expect("expected text content");
