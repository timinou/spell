...
fn ts_dialect_dispatch() {
	...
	let chunks = execute_code_path_inner(
		opts_with_root("foo.ts::Foo.bar.baz", root),
		CancelToken::default(),
	)
	.unwrap();
...
fn unknown_extension_fallback() {
	...
	let chunks = execute_code_path_inner(
		opts_with_root("foo.unknown::Bar", root),
		CancelToken::default(),
	)
	.unwrap();
	let nodes: Vec<_> = chunks.iter().flat_map(|c| c.nodes.iter()).collect();
...
fn bare_path_no_dispatch() {
	...
	let chunks = execute_code_path_inner(
		opts_with_root("src/foo.go", root),
		CancelToken::default(),
	)
	.unwrap();
	let nodes: Vec<_> = chunks.iter().flat_map(|c| c.nodes.iter()).collect();
...
fn parse_round_trip_rust_path() {
	...
	let name_payload = head_head.get("Name").and_then(|n| n.get("Raw")).and_then(|r| r.as_str()).unwrap();
