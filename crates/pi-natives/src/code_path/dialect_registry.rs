...
use pi_code_path::dialect::NameLexer;
use pi_code_path::dialects::{
...
pub fn select_dialect(path: &Path) -> Option<Arc<dyn NameLexer>> {
    ...
    match ext {
        "ts" | "tsx" | "js" | "jsx" | "mjs" | "cjs" => {
            Some(TS_LEXER.get_or_init(|| typescript_dialect().name_lexer).clone())
        }
        "rs" => Some(RUST_LEXER.get_or_init(|| rust_dialect().name_lexer).clone()),
        "py" => Some(PYTHON_LEXER.get_or_init(|| python_dialect().name_lexer).clone()),
        "go" => Some(GO_LEXER.get_or_init(|| go_dialect().name_lexer).clone()),
        "hs" | "lhs" => Some(HASKELL_LEXER.get_or_init(|| haskell_dialect().name_lexer).clone()),
        ...
        "md" | "mdx" | "org" => Some(MDORG_LEXER.get_or_init(|| markdown_dialect().name_lexer).clone()),
        _ => None,
    }
}
