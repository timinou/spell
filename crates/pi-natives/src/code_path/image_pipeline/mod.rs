...
/// Resolver that provides image pipeline services for `#image` and
/// `#thumbnail[N]`.
...
/// Returns a diagnostic recommending the `inspect_image` tool for visual
/// analysis.
...
mod tests {
	...
	fn large_png_triggers_resize_flag_with_low_threshold() {
		...
		let opts = ImageLoader { inline_threshold_bytes: 1024, ..Default::default() };
		let loaded = load(&path, &opts).unwrap();
...
"#,
	...
	fn max_bytes_rejected() {
		...
		let opts = ImageLoader { inline_threshold_bytes: 1024, max_bytes: 1024 };
		let err = load(&path, &opts).unwrap_err();
		assert!(matches!(err.variant, DiagnosticVariant::ParseError));
