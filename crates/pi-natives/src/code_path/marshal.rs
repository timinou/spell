...
pub fn diagnostic_variant_to_string(v: &DiagnosticVariant) -> String {
	match v {
		...
		DiagnosticVariant::Informational => "informational".to_string(),
	}
}
...
fn sniff_image_mime(bytes: &[u8]) -> Option<&'static str> {
	...
	if bytes.len() >= 12 && bytes[0..4] == [82, 73, 70, 70] && bytes[8..12] == [87, 69, 66, 80] {
...
mod tests {
	...
	fn image_mime_corrected_from_bytes() {
		...
		assert_eq!(
			...
			Some("image/jpeg"),
			"sniffer should detect JPEG magic over declared png"
		);
	}
}
