# System Prompt Rewrite — Full Template

This is the proposed replacement for `packages/coding-agent/src/prompts/system/system-prompt.md`.
All `{{...}}` blocks are Handlebars template syntax and must be preserved exactly.
...
- `{{#if cavemanThinking}}` → `{{#if terseThinking}}`
- Requires `system-prompt.ts` to compute `terseThinking: cavemanThinkingMode !== "normal"` (decoupled from `cavemanActive`)

### Identity section changes

**`<behavior>`** — restored two adversarial questions:
- "What would a malicious caller do?" (security check)
- "Would a tired maintainer misunderstand this?" (readability check)

**`<thinking-mode>`** — now unconditional (default on for all sessions):
- Controlled by `terseThinking` instead of `cavemanThinking`
- No longer requires caveman output mode to be active
- Content unchanged — symbols, structure markers, three examples

**`<code-integrity>`** — expanded outside-in framework:
- **Callers** bullet: added "plausible-looking output" failure mode
- **System** bullet: added "not standalone" framing, concrete propagation examples
- **Time** bullet: added cost-blindness observation, concrete examples
- New: pipeline consumer heuristic ("what does the next consumer need?")
- New: "DRY at 2" with rationale
- Expanded: maintainability guidance (comments explain why)
- Expanded: "Earn every line" with second example (test-access wrapper)

**`# Design Integrity`** — consolidated with Procedure philosophy (5 → 8 principles):
- Existing 5 principles expanded to full voice (each gets the "why" not just the "what")
- 3 new principles absorbed from Procedure ## 5:
  - "One job, one level of abstraction"
  - "Fix where the invariant is violated"
  - "After writing, inhabit the call site"
- 2 Procedure principles merged into existing bullets:
  - "New code makes old code obsolete" → absorbed into "One concept, one representation"
  - "No forwarding addresses" → absorbed into "One concept, one representation"
- Opening expanded: "Every vestige of old design left compilable and reachable is a lie told to the next reader"

### Procedure section changes

**## 5. While Working** — stripped to operational core:
- Keeps opening sentence: "You are not making code that works..."
- Philosophy removed (moved to Design Integrity)
- Only operational instructions remain: tool failure handling, destructive command guards, concurrency

### Code changes required (outside template)

1. `system-prompt.ts` line 513: `cavemanThinking` → `terseThinking: cavemanThinkingMode !== "normal"`
2. `system-prompt.ts`: add `terseThinking` to template data object
3. `settings-schema.ts` line 458: update description to "Thinking block style: compressed symbolic notation (default) or verbose prose"
4. Test assertions: update strings that check for `"Think in notation"`, `"Symbols carry logic"`, `"Terse mode active"` positioning
